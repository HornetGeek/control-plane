...
**Input**: Design documents from `/specs/001-control-plane-mvp/`
...
- [X] T002 Initialize Python project with pyproject.toml, dependencies (FastAPI, SQLAlchemy, pytest, etc.)
- [X] T003 [P] Configure pre-commit hooks (black, ruff, mypy) in .pre-commit-config.yaml
- [X] T004 [P] Setup pytest configuration in pyproject.toml
- [X] T005 [P] Create .env.example with all required environment variables
- [X] T006 Create base FastAPI application in src/main.py with /health and /ready endpoints
...
- [X] T007 Setup async SQLAlchemy engine and session factory in src/db/session.py
- [X] T008 Create base declarative model with TimestampMixin in src/models/base.py
- [X] T009 [P] Create Organization model in src/models/organization.py
- [X] T010 [P] Create Tenant model in src/models/tenant.py
- [X] T011 [P] Create User model in src/models/user.py
- [X] T012 [P] Create Membership model in src/models/membership.py
- [X] T013 [P] Create Application model in src/models/application.py
- [X] T014 [P] Create Subscription model in src/models/subscription.py
- [X] T015 Configure Alembic for migrations in alembic/env.py

### Security Infrastructure (Foundation for all protected endpoints)

- [X] T016 Create OIDC token validation in src/security/oidc.py (JWKS fetch, signature verification)
- [X] T017 Create authorization helpers in src/security/authorization.py (role checking, org/tenant access)
- [X] T018 Create FastAPI auth dependency in src/api/dependencies.py (token validation, user attachment)
- [X] T019 Create audit stubs in src/security/audit.py (internal interfaces, minimal logging)

### API Infrastructure (Foundation for all endpoints)

- [X] T020 Create error response schemas in src/schemas/common.py (standard error format)
- [X] T021 Create correlation ID middleware in src/api/middleware.py (X-Request-ID handling)
- [X] T022 Create exception handlers in src/main.py (transform exceptions to error responses)
- [X] T023 Create pagination utilities in src/schemas/common.py (limit/offset params, response wrapper)

### Configuration

- [X] T024 Create configuration class in src/config.py using pydantic-settings (all env vars)
- [X] T025 Create Alembic initial migration generating all tables
...
- [ ] T031 [P] [US1] Contract test for /v1/auth endpoints in tests/contract/test_v1_auth.py
...
- [ ] T044 [P] [US2] Contract test for /v1/tenants endpoints in tests/contract/test_v1_tenants.py
...
- [ ] T057 [P] [US3] Contract test for /v1/memberships endpoints in tests/contract/test_v1_memberships.py
...
- [ ] T068 [P] [US4] Contract test for /v1/subscriptions endpoints in tests/contract/test_v1_subscriptions.py
...
- [ ] T081 [P] [US5] Contract test for /v1/launch endpoint in tests/contract/test_v1_launch.py
...
- [ ] T085 [US5] Create application seed script in src/db/seed.py (PACS, ERP)
...
- [ ] T086 [P] Create README.md with project overview, setup instructions, and API documentation link
- [ ] T087 [P] Create docs/quickstart.md with detailed local development setup
- [ ] T088 [P] Create docs/deployment.md with container and deployment guidance

### Data Seeding

- [ ] T089 Create organization seed script in src/db/seed_organizations.py (initial test orgs)
- [ ] T090 Create user seed script in src/db/seed_users.py (initial test users with OIDC subs)

### Testing & Quality

- [ ] T091 [P] Add unit tests for OIDC validation in tests/unit/test_security.py
- [ ] T092 [P] Add unit tests for authorization helpers in tests/unit/test_security.py
- [ ] T093 [P] Add unit tests for all models in tests/unit/test_models.py
- [ ] T094 Run full test suite and ensure 90%+ coverage
- [ ] T095 Verify OpenAPI schema generation is complete and accurate

### Security Hardening

- [ ] T096 Verify all authorization failures are logged with correlation ID
- [ ] T097 Verify error messages don't leak internal details
- [ ] T098 Verify SQL injection protection via parameterized queries (SQLAlchemy default)

### Performance

- [ ] T099 Load test /v1/auth/login endpoint (target: 1000 concurrent requests)
- [ ] T100 Verify database indexes are created via Alembic migration
- [ ] T101 Verify JWKS caching is working (check OIDC service logs)
...
Task: "Contract test for /v1/tenants endpoints in tests/contract/test_v1_tenants.py"
```
...
| Phase 2: Foundational | 19 tasks | Blocking infrastructure |
| Phase 3: User Story 1 | 12 tasks | Authentication (P1) |
| Phase 4: User Story 2 | 13 tasks | Tenant Management (P1) |
| Phase 5: User Story 3 | 10 tasks | Membership Management (P2) |
| Phase 6: User Story 4 | 13 tasks | Subscription Management (P2) |
| Phase 7: User Story 5 | 10 tasks | Application Launch (P3) |
| Phase 8: Polish | 16 tasks | Cross-cutting concerns |
| **Total** | **99 tasks** | Complete MVP implementation |

### Parallel Opportunities

- **30 tasks marked [P]** can run in parallel within their phases
- **24 test tasks** can all be written in parallel (by phase/story)
- **6 model tasks** in Foundational phase can run in parallel

### Independent Test Criteria

| User Story | Test Criteria |
|------------|---------------|
| US1: Authentication | Complete OIDC flow creates/updates user with correct org |
| US2: Tenant Management | Org admin can create, list, and view tenants |
| US3: Membership Management | Admins can add/remove users with roles |
| US4: Subscription Management | Admins can subscribe tenants idempotently |
| US5: Launch | Tenant member receives redirect URL for subscribed apps |
